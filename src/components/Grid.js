...
export default class Grid extends Component<Props, State> {
  ...
  initializeFromProps(props: Props, fromConstructor: boolean) {
    ...
    let state = {
      ...
      decreaseInEncountersAfterSymptoms:
        props.decreaseInEncountersAfterSymptoms,
  ...
  generate(force: boolean) {
    ...
    if (
      !force &&
      Utils.arraysEqual(this.previousSimulationParams, currentSimulationParams)
    ) {
    ...
    if (
      this.state.infectedPerDay.length > 1 &&
      this.state.infectedPerDay[this.state.infectedPerDay.length - 2] !== null
    ) {
    ...
    if (
      this.state.infectedPerDay.length === 0 ||
      this.state.infectedPerDay[this.state.infectedPerDay.length - 1] === null
    ) {
      this.state.capacityPerDay.push(
        this.state.hospitalCapacityPct *
          this.props.gridRows *
          this.props.gridRows
      );
    ...
    if (this.props.addCities) {
      let cityCenters = [];
      cityCenters.push([
        Math.floor((3 / 4) * nRows),
        Math.floor((1 / 4) * nCols)
      ]);
      cityCenters.push([
        Math.floor((1 / 4) * nRows),
        Math.floor((3 / 4) * nCols)
      ]);
      for (let r = 0; r < nRows; r++) {
        for (let c = 0; c < nCols; c++) {
  ...
  simulateStep() {
    ...
    for (let r = 0; r < nRows; r++) {
      for (let c = 0; c < nCols; c++) {
        let node = this.grid[r][c];
        if (
          this.props.showInteractions &&
          this.isCenterNode(r, c) &&
          node.canInfectOthers()
        ) {
          centerNodeNeighborsToDisplay = this.maybeInfect(
            node,
            r,
            c,
            linkedNodes
    ...
    for (let r = 0; r < nRows; r++) {
      for (let c = 0; c < nCols; c++) {
        let node = this.grid[r][c];
        if (
          node.getNextState() === Constants.EXPOSED ||
          node.getNextState() === Constants.INFECTED
        ) {
          actualInfectedNodes++;
        }
      }
    }
    let chanceOfIsolationAfterSymptoms = this.state
      .chanceOfIsolationAfterSymptoms;
    if (!this.props.showChanceOfIsolationAfterSymptomsSlider) {
      chanceOfIsolationAfterSymptoms = 0;
    }
    let overCapacity =
      this.state.hospitalCapacityPct > -1 &&
      actualInfectedNodes > this.state.hospitalCapacityPct * (nRows * nCols);
    for (let r = 0; r < nRows; r++) {
      for (let c = 0; c < nCols; c++) {
        let node = this.grid[r][c];
        node.endDay(
    ...
    this.state.capacityPerDay.push(
      this.state.hospitalCapacityPct * this.props.gridRows * this.props.gridRows
    );
  ...
  redraw(force: boolean) {
    ...
    if (
      !force &&
      Utils.arraysEqual(this.previousDrawingParams, currentDrawingParams)
    ) {
      // console.log('no draw');
      return;
    }
    ...
    let interactionsParamsChanged = !Utils.arraysEqual(
      this.previousInteractionsParams,
      showInteractionsParams
    );
    ...
    if (
      this.props.showInteractions &&
      (interactionsParamsChanged || this.inInitialPosition())
    ) {
  ...
  drawCell(r: number, c: number, node: GridNode, context, highlight: boolean) {
    ...
    if (node.isExposed()) {
      ...
      if (node.specialDegree !== null) {
        ...
        Utils.assert(
          node.specialDegree >= 4 && node.specialDegree <= 8,
          "node.specialDegree should be between 4 and 8; was: " +
            node.specialDegree
        );
        let intensity = (node.specialDegree - 4) / 4.0;
        context.fillStyle = Colors.hex(
          Colors.blend(
            Colors.makeHex(Grid.SUSCEPTIBLE_COLOR),
            Colors.makeHex("#BBB"),
            intensity
          )
        );
  ...
  render() {
    ...
    if (showAll || this.props.showTransmissionProbabilitySlider) {
      transmissionProbabilitySlider = this.renderSlider(
        "Швидкість передачі інформації",
        this.state.transmissionProbability,
        (e, value) => {
          this.setState({ transmissionProbability: value });
        },
        0,
        this.props.maxTransmissionRate,
        0.01,
        false,
        this.props.highlight === "transmissionRate"
      );
    }
    ...
    if (showAll || this.props.showHospitalCapacitySlider) {
      ...
      hospitalCapacitySlider = this.renderSlider(
        "Достовірність",
        this.state.hospitalCapacityPct,
        (e, value) => {
          this.setState({ hospitalCapacityPct: value });
        },
        0,
        1,
        0.01,
    ...
    if (showAll || this.props.showTravelRadiusSlider) {
      travelRadiusSlider = this.renderSlider(
        "Радіус комунікації",
        this.state.travelRadius,
        (e, value) => {
          this.setState({ travelRadius: value });
        },
        0,
        Math.min(30, Math.floor(this.props.gridRows / 2)),
        1,
        false,
        false
      );
    ...
    if (showAll || this.props.showPersonHoursSlider) {
      personHoursSlider = this.renderSlider(
        "Кількість зустрічей, розмов, контактів за день",
        this.state.personHours,
        (e, value) => {
          this.setState({ personHours: value });
        },
        1,
        30,
        1,
    ...
    if (showAll || this.props.showDaysPerStateControls) {
      daysIncubatingSlider = this.renderSlider(
        "Період сприйняття інформації",
        this.state.daysIncubating,
        (e, value) => {
          this.setState({ daysIncubating: value });
        },
        0,
        20,
        1,
    ...
    if (showAll || this.props.showDaysPerStateControls) {
      daysSymptomaticSlider = this.renderSlider(
        "Період усвідомлення інформації",
        this.state.daysSymptomatic,
        (e, value) => {
          this.setState({ daysSymptomatic: value });
        },
        1,
        20,
        1,
    ...
    if (showAll || this.props.showChanceOfIsolationAfterSymptomsSlider) {
      chanceOfIsolationAfterSymptomsSlider = this.renderSlider(
        "Рівень критичності",
        this.state.chanceOfIsolationAfterSymptoms,
        (e, value) => {
          this.setState({ chanceOfIsolationAfterSymptoms: value });
        },
        0,
        1,
        0.01,
    ...
    if (showAll || this.props.showDecreaseInEncountersAfterSymptomsSlider) {
      decreaseInEncountersAfterSymptomsSlider = this.renderSlider(
        "Загальний рівень освіченості",
        this.state.decreaseInEncountersAfterSymptoms,
        (e, value) => {
          this.setState({ decreaseInEncountersAfterSymptoms: value });
        },
        0,
        1,
        0.01,
    ...
    if (showAll || this.props.showDeathRateSlider) {
      ...
      if (this.state.hospitalCapacityPct > -1) {
        sliderName = "Рівень впливу ";
      }

      deathRateSlider = this.renderSlider(
        sliderName,
        this.state.deathRate,
        (e, value) => {
          this.setState({ deathRate: value });
        },
        0,
        0.3,
        0.01,
      ...
      toggleLongDistanceNetwork = (
        <div>
          <span
            onClick={() => {
              this.setState({
                longDistaceNetworkActive: !this.state.longDistaceNetworkActive
              });
            }}
          >
            {text}
    ...
    if (this.props.showProTip) {
      protip = (
        <div style={{ color: "#666", fontSize: "12pt", marginTop: "1em" }}>
    ...
    if (this.props.showAliveFraction) {
      ...
      plot = (
        <Plot
          hospitalCapacity={this.state.hospitalCapacityPct * population}
